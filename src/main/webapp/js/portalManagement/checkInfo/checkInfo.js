...
{
	commonGetrowdatas("CheckInfoGrid",getSearchData(),contextPath + "/PortalManagement/CheckInfo/search","commonCallback",true);
}
...
function checkPass(contentId)
{
	...
	doAjax("post",contextPath + "/PortalManagement/CheckInfo/checkPass",data,"checkPassSuccess",null,false);
}
...
function checkBack(contentId)
{
	...
	doAjax("post",contextPath + "/PortalManagement/CheckInfo/checkBack",data,"checkPassSuccess",null,false);
}
...
function checkView(contentId)
{
	top.$('#myDialog').dialog({
	    ...
	    closed: false,
	    cache: false,
	    href: contextPath + "/PortalManagement/CheckInfo/checkView?contentId=" + contentId,
	    modal: true
	});
}
...
function batchCheckPass()
{
	...
	if(selectRows>0)
	{
		...
		doAjax("post",contextPath + "/PortalManagement/CheckInfo/batchCheckPass",data,"checkPassSuccess",null,false);
	}else{
...
function batchCheckBack()
{
	...
	if(selectRows>0)
	{
		...
		doAjax("post",contextPath + "/PortalManagement/CheckInfo/batchCheckBack",data,"checkPassSuccess",null,false);
	}else{
		showOnlyMessage("error",getMessageFromList("SelectRow",null))
	}
}
