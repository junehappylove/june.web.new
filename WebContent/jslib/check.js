...
function checkMustInput(id,msgParams){
	...
	{
		var errMsg = getMessageFromList("ErrorMustInput",[msgParams]);
		appendErrMsg(errMsg);
		if(firstControlId=="")
		{
			firstControlId = id;
...
function checkMustSelect(id,msgParams)
{
	...
	if(value < 0 || value=="--选择--" || value.replace(/(^\s*)|(\s*$)/g, "")=="")
	{
		var errMsg = getMessageFromList("ErrorMustSelect",[msgParams]);
		appendErrMsg(errMsg);
		if(firstControlId=="")
		{
			firstControlId = id;
...
function isInteger(id,msgParams){
	...
	if (value != "") {
		...
		if(regu.test(value)){
			...
			var errMsg = getMessageFromList("ErrorInteger",[msgParams]);
			appendErrMsg(errMsg);
			if(firstControlId=="")
			{
				firstControlId = id;
...
function checkMobilPhone(id,msgParams)
{
	...
	if (value != "") {
		...
		if (reg.test(value)) {
			...
			var errMsg = getMessageFromList("ErrorMobile",[msgParams]);
			appendErrMsg(errMsg);
			if(firstControlId=="")
			{
				firstControlId = id;
...
function isEmail(id,msgParams){  
	...
	if(myReg.test(value))
	{
		...
		var errMsg = getMessageFromList("ErrorEmail",[msgParams]);
		appendErrMsg(errMsg);
		if(firstControlId=="")
		{
			firstControlId = id;
...
function checkDate(id,msgParams) {
	...
	if (value != "") {
		...
		if (!regDate.test(value)) {
			// {0}输入的日期类型格式不正确。
			var errMsg = getMessageFromList("ErrorDate",[msgParams]);
			appendErrMsg(errMsg);
			return false;
		}
...
function checkTwoDate( startDateid,endDateid,msgParams1,msgParams2) {
	...
	if( !checkDate(startDateid,msgParams1) ) { 
		...
		var errMsg = getMessageFromList("ErrorDateCopmare",[msgParams1,msgParams2]);
		appendErrMsg(errMsg);
		if(firstControlId=="")
		{
			firstControlId = startDateid;
...
function checkTelphone(id,msgParams) {
	...
	if (value != "") {
		...
		if (reg.test(value)) {
			...
			var errMsg = getMessageFromList("ErrorTelphone",[msgParams]);
			appendErrMsg(errMsg);
			if(firstControlId=="")
			{
				firstControlId = id;
...
function checkAlphaMark(id,msgParams)
{
	...
	if (value != "") {
		for (i = 0; i < value.length; i++) {
			...
			if (code <33 || code > 126) {
				// {0}只能输入半角英数字和符号。
				appendErrMsg(getMessageFromList("ErrorLength", [msgParams]));
				if(firstControlId=="")
				{
					firstControlId = id;
				}
...
function maxLength(id,val,msgParams1,msgParams2){
	...
	if(length>val)
	{
		appendErrMsg(getMessageFromList("ErrorLength", [msgParams1,msgParams2]));
		if(firstControlId=="")
		{
			firstControlId = id;
		}
...
function checkNum(id,msgParams)
{
	...
	if(myReg.test(value))
	{
		...
		var errMsg = getMessageFromList("Errornum",[msgParams]);
		appendErrMsg(errMsg);
		if(firstControlId=="")
		{
			firstControlId = id;
...
function checkExcelFile(id,msgParams)
{
	...
	if(filename!="")
	{
		...
		if(filename == "xlsx" || filename == "xls")
		{
			...
			var errMsg = getMessageFromList("ErrorExceleFile",[msgParams]);
			appendErrMsg(errMsg);
			if(firstControlId=="")
			{
				firstControlId = id;
...
function checkEditRows(rows)
{
	if(rows==0)
	{
		var errMsg = getMessageFromList("SelectRow");
		appendErrMsg(errMsg);
		return false;
	}
	else if(rows > 1)
	{
		var errMsg = getMessageFromList("editmultiselectError");
		appendErrMsg(errMsg);
		return false;
	}else{
		return true;
...
function addErrMsg(msgType,msgParams){
	var errMsg = getMessageFromList(msgType,[msgParams]);
	appendErrMsg(errMsg);
	return false;
}
...
{
	  var filename=$("#"+id).val();
	  var fileType=filename.substring(filename.lastIndexOf('.'),filename.lenth).toLowerCase(); 
	   if(fileType=='.bmp' || fileType=='.gif' || fileType=='.jpeg' || fileType=='.png' || fileType=='.jpg') 
       {
           return true;
       }else{
    	   var errMsg = getMessageFromList("ErrorImageType",[msgParams]);
			appendErrMsg(errMsg);
			if(firstControlId=="")
			{
				firstControlId = id;
...
function checkIdentityCard(inputvalue) {
	if (!(isInteger(inputvalue))||!(isLength(inputvalue))||!(isLegal(inputvalue))) {
		var errMsg = getMessageFromList("IdCardIllegal");
		appendErrMsg(errMsg);
		if(firstControlId=="")
		{
			firstControlId = id;
...
function setFocus(id)
{
	if($("#" + id).length>0)
	{
		//根据id获取控件的class属性
	  	...
	  	}else if(classAttr.split(' ')[0]=="easyui-datebox"||classAttr.split(' ')[0]=="easyui-datetimebox")
	  	{
	  		easyuiDateFocus(id);
	  	}
	}
